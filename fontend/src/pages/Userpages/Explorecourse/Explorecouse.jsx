...
console.log(filter);
  ...
  const getcourse = async() => {
    ...
    await axios.get("http://localhost:8000/courses/"+localStorage.getItem("Userid"),{
      headers: {
        Authorization: `Bearer ${localStorage.getItem("Token")}`,
      },
    }).then(
...
const payment= async(amount,courseid)=>{
  ...
  await axios.get("http://localhost:8000/transaction/"+amount,{
    headers: {
      Authorization: `Bearer ${localStorage.getItem("Token")}`,
    },
  }).then(
  ...
  const options = {
    ...
    currency: "INR",
    name: "Brocode Learning Platform",
 ...
 console.log(paymentId.current);
  ...
  const endroll = (courseid) => {
    ...
    axios
      .post(`http://localhost:8000/enroll?userid=${localStorage.getItem("Userid")}&courseId=${courseid}`,{},{
        headers: {
          Authorization: `Bearer ${localStorage.getItem("Token")}`,
        }
      })
  ...
  return (
    <div className="parentcoursediv">
      ...
      <div className="search">
        ...
        >
          {" "}
          <Singlebutton url={Dashboardicon.mi} backcolor="white" />{" "}
        </div>
      </div>
